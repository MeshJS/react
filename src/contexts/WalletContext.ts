...
const INITIAL_STATE = {
  ...
  walletInstance: {} as MeshCardanoBrowserWallet,
};
...
export const useWalletStore = () => {
  ...
  const [connectedWalletInstance, setConnectedWalletInstance] =
    useState<MeshCardanoBrowserWallet>(INITIAL_STATE.walletInstance);
  ...
  const connectWallet = useCallback(
    async (walletName: string, persist?: boolean) => {
      ...
      try {
        const extensions = (globalThis as any)?.cardano?.[walletName]?.supportedExtensions ?? [];
  ...
  const setWallet = useCallback(
    async (
      ...
      const wrapped = walletInstance instanceof MeshCardanoBrowserWallet
        ? walletInstance
        : new MeshCardanoBrowserWallet(walletInstance);
  ...
  useEffect(() => {
    async function load() {
      if (
        ...
        if (!address)
          address = await connectedWalletInstance.getChangeAddressBech32();
        setAddress(address);
      }
    }
    load();
